...
- `rounded-3xl`: guestbook spread, modals
...
**Guestbook spread** — `rounded-3xl` open-book card, two columns + divider line, Lora italic message, green signature line, faint small-caps date. Pending/scheduled entries render fully muted (`state-pending`) — real status, not just opacity. "Load Older Messages": outline pill.
...
- Guestbook = single large "book" card, not a list feed
